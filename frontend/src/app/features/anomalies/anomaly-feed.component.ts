...
export class AnomalyFeedComponent {
  ...
  protected metric(alert: AnomalyAlert) { return CHART_METRICS.find(metric => metric.key === alert.reading.metricType); }
  ...
  protected focus(alert: AnomalyAlert): void {
    ...
    chart?.scrollIntoView({ block: 'start', behavior: this.document.defaultView?.matchMedia('(prefers-reduced-motion: reduce)').matches ? 'instant' : 'smooth' });
  }
}
