...
const empty = (source: AnomalyFeedState['source'], message = ''): AnomalyFeedState => ({ source, alerts: [], message, updatedAt: null });
...
export class AnomalyFeedStore {
  ...
  readonly feed = toSignal(toObservable(this.availability).pipe(
    switchMap(({ token, health }) => {
      ...
      return timer(0, 5_000).pipe(
        exhaustMap(() => {
          const to = new Date();
          return this.http.get<AnomalyAlert[]>('/api/anomalies', { params: {
            from: new Date(to.getTime() - 3_600_000).toISOString(), to: to.toISOString(), limit: 100, offset: 0,
          } }).pipe(
            timeout(8_000),
            map(alerts => ({ source: 'live' as const, alerts: [...new Map(alerts.map(alert => [alert.reading.id, alert])).values()]
              .sort((a, b) => Date.parse(b.reading.timestamp) - Date.parse(a.reading.timestamp) || b.reading.id - a.reading.id), message: '', updatedAt: Date.now() })),
  ...
  private demo(): AnomalyFeedState {
    return { source: 'demo', alerts: this.fixtures, message: 'Demo alerts — synthetic examples, not live machine data. Backend unavailable.', updatedAt: null };
  }
}
