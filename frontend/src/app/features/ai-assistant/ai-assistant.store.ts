...
function failure(error: unknown): string {
    ...
    if (error instanceof HttpErrorResponse) {
        switch (error.status) {
            ...
            case 403:
                return 'An administrator account is required.';
            case 404:
                return 'This conversation or AI endpoint is unavailable. For chat, start a new conversation.';
            case 409:
                return 'A reply is already being generated for this conversation. Wait before sending again.';
            case 503:
                return 'AI is unavailable. Check that RAG, the agent, and their model services are enabled.';
            case 400:
                return 'The question could not be accepted. Use between 1 and 2,000 characters.';
        }
    }
...
export class AiAssistantStore {
    ...
    search(): void {
        ...
        this.ragRequest = this.http.post<RagQueryResponse>('/api/rag/query', {question}).pipe(
            ...
            next: answer => {
                if (token === this.auth.accessToken()) this.ragAnswer.set(answer);
            },
            error: error => {
                if (token === this.auth.accessToken()) this.ragError.set(failure(error));
            },
        });
    }
    ...
    send(): void {
        ...
        this.chatRequest = this.http.post<AgentChatResponse>('/api/agent/chat', request).pipe(
            ...
            error: error => {
                if (token !== this.auth.accessToken()) return;
                this.chatError.set(failure(error));
                this.chatQuestion.set(question);
            },
        });
    }
