...
import { defer, finalize, of, tap, throwError, timeout } from 'rxjs';
...
export class AuthService {
  ...
  openLogin(): void { this.modal.set(true); }
  closeLogin(): void { this.modal.set(false); }
  ...
  login(credentials: LoginCredentials) {
    return defer(() => {
      ...
      return this.http.post<AuthResponse>('/api/auth/login', credentials).pipe(
        ...
        finalize(() => { if (generation === this.generation) this.pending.set(false); }),
      );
    });
  }
  ...
  private scheduleExpiry(): void {
    ...
    if (remaining <= 0) { this.requireLogin(this.accessToken()); return; }
